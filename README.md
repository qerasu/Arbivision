...
7. Worker создаёт `Alert`, сразу пытается отправить свежие delivery и затем коммитит итоговый статус; Telegram loop остаётся путём для queued/retry recovery и повторной доставки. Recovery `fanout` loop при отдельном запуске разбирает queued/retry `ArbOpportunity` батчами.
...
`all` рассчитан на основной сценарий, где worker пытается быстро доставить свежий alert, а отдельный Telegram loop подбирает queued/retry записи как recovery-контур.
...
- за один worker-цикл проверяется не больше `MAX_ACTIVE_PAIRS_PER_CYCLE` пар; пары с ближайшим временем закрытия получают приоритет, а внутри переполненных очередей используется ротация между циклами, чтобы хвост не голодал
